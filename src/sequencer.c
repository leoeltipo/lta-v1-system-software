...
int sequencer_load_program(sequencer_t *sequencer)
{
	...
	{
		// Stop the sequencer
		SEQUENCER_mWriteReg(XPAR_SEQUENCER_S00_AXI_BASEADDR,SEQUENCER_STOP_SEQUENCE_OFFSET, 0x0);

		// Load program.
		for (int i = 0; i < n; ++i) {
			SEQUENCER_mWriteReg(XPAR_SEQUENCER_S00_AXI_BASEADDR,SEQUENCER_ADDR_OFFSET, (uint32_t)i);
			SEQUENCER_mWriteReg(XPAR_SEQUENCER_S00_AXI_BASEADDR,SEQUENCER_DATA_OFFSET, sequencer->program[i]);
			SEQUENCER_mWriteReg(XPAR_SEQUENCER_S00_AXI_BASEADDR,SEQUENCER_WEA_OFFSET, 0x00000001);
			SEQUENCER_mWriteReg(XPAR_SEQUENCER_S00_AXI_BASEADDR,SEQUENCER_WEA_OFFSET, 0x00000000);
		}
...
int seq_change_sw_status(seq_sw_status_t *seq_sw, uint8_t value)
{
	...
	SEQUENCER_mWriteReg(XPAR_SEQUENCER_S00_AXI_BASEADDR, seq_sw->reg_offset,(uint32_t) seq_sw->status);
...
{
	SEQUENCER_mWriteReg(XPAR_SEQUENCER_S00_AXI_BASEADDR,SEQUENCER_STOP_SEQUENCE_OFFSET, 0x1);
}
...
{
	volatile uint32_t end = SEQUENCER_mReadReg(XPAR_SEQUENCER_S00_AXI_BASEADDR, SEQUENCER_EOS_OFFSET);
